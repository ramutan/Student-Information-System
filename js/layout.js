/* layout.js — builds the sidebar + topbar on every app page */
(function () {
    ...
    document.getElementById('sidebar').innerHTML = `
        <div class="side-brand">
            ...
            <div><h1>REGISTRAR</h1><p>SIS PORTAL</p></div>
        </div>
        <nav>
            ${links.map(l => `<a href="${l.href}" class="${l.href === here ? 'active' : ''}"><span class="num">${l.num}</span>${l.label}</a>`).join('')}
        </nav>
        <div class="side-foot">
            ${user.photo ? `<img class="side-avatar" src="${user.photo}" alt="">` : ''}Signed in as<br><b>${SIS.esc(displayName)}</b>
        </div>`;
    ...
    document.getElementById('topbar').innerHTML = `
        ...
        <div class="top-right">
            ...
            <button type="button" class="btn ghost sm icon-btn theme-toggle" onclick="SIS.toggleTheme()" title="Light / dark mode"></button>
